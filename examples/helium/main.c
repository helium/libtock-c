...
int main(void) {
    ...
    printf("TI Sensor BoosterPack and Helium");
    ...
    uint8_t bme = bme280_get_sample_forced_mode(&bme280);
    uint8_t bmi = bmi160_get_all_with_time(&bmi160);
    uint8_t bmm = bmm150_get_data(&bmm150);
    bool lux = opt3001_get_lux(&opt3001);

    char message[157]; 
    sprintf(message, "{\"team\":\"%s\",\"payload\":{\"bme\":%d,\"bmi\":%d,\"lux\":%d,\"bmm\":%d}}", address,bme,bmi,lux,bmm);
